...
"""
        ...
        for argPosition, cmndArgSpec in six.iteritems(cmndArgsSpecDictDict):
...
"""
    ...
    def out(self):
        ...
        if self.stdout: OUT_write(self.stdout.decode("utf-8"))     # PY2.7           
...
def loadFile(file):
    ...
    exec(compile(open(file).read(), file, 'exec'), globdict)
...
def subProc_bash(
    ...
    try:
        ...
        #outcome.stdout, outcome.stderr = process.communicate(input=format(stdin.encode())) PY2.7
        outcome.stdout, outcome.stderr = process.communicate(input=stdin.encode())
        process.stdin.close()
...
def FILE_paramDictRead(interactive=Interactivity.Both,
    ...
    for thisPath in inPathList:
        ...
        for parName, filePar  in six.iteritems(thisParamDict):
...
def FILE_paramDictPrint(fileParamDict):
    """ Returns a Dictionary of paramName:FILE_Param.        """
    for parName, filePar  in six.iteritems(fileParamDict):
        #print('parName=' + parName)
        if filePar == None:
            continue
        thisValue=filePar.parValueGetLines()
...
**  [[elisp:(org-cycle)][| ]]  Subject      :: type= is missing -- [[elisp:(org-cycle)][| ]]
    ...
    for key, icmParam in six.iteritems(icmParams.parDictGet()):
...
def cmndParamsMandatoryAssert(paramsList):
    for key, value in six.iteritems(paramsList):
...
"""
        ...
        for key, modPath in six.iteritems(importedCmnds):
